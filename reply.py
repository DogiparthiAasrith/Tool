...
def process_follow_ups(db):
    ...
    pipeline = [
        ...
        {'$match': {
            # --- CORRECTED LOGIC ---
            # Condition 1: They must have received an 'initial_outreach' email.
            'events': {'$in': ['initial_outreach']},
            # Condition 2: They must NOT have replied or been sent a follow-up.
            'events': {'$nin': ['follow_up_sent', 'replied_positive', 'replied_negative', 'replied_neutral']},
            # Condition 3: The first contact must have been before our waiting period.
...
def process_unsubscribes(db):
    ...
    pipeline = [
        # --- CORRECTED LOGIC ---
        # Count both the first email and any follow-ups.
